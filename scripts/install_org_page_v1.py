#!/usr/bin/env python3
"""Install app/ui/pages/org_page.py from zlib+base64 payload."""
from __future__ import annotations
import base64, zlib, re
from pathlib import Path
ROOT = Path(__file__).resolve().parents[1]
PAYLOAD = Path(__file__).resolve().parent / "refactor_payload"
OUT = ROOT / "app/ui/pages/org_page.py"
def main() -> None:
    parts = sorted(p for p in PAYLOAD.glob("org_page_v1.z*.b64") if re.search(r"\.z\d{2}\.b64$", p.name))
    if not parts:
