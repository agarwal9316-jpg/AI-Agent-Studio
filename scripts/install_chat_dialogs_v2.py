#!/usr/bin/env python3
"""Install chat_dialogs from verified zlib+base64 payload chunks (v2)."""
...
def main() -> None:
    parts = sorted(p for p in PAYLOAD.glob("chat_dialogs_v2.z*.b64") if re.search(r"\.z\d{2}\.b64$", p.name))
    if not parts:
